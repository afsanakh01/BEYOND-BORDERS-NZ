...
const unis = [
  {
    ...
    desc: "A globally recognised American-curriculum college in Cyprus. Business, IT, Law and Hospitality programs.",
    img: "/cyprus.jpg",
  },
  {
...
export default function Universities() {
  return (
    <section id="universities" style={{ background: "#0c0c0c", padding: "8rem 4rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1.5px", background: "rgba(200,169,110,0.15)" }}>
        {unis.map((u, i) => (
          <div
            key={i}
            style={{ position: "relative", height: "520px", overflow: "hidden", cursor: "pointer" }}
            onMouseEnter={e => {
          ...
          >
            ...
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "2.2rem" }}>
              ...
              <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.7rem", fontWeight: 400, color: "#FAFAF8", lineHeight: 1.15, marginBottom: "0.3rem" }}>{u.name}</h3>
              ...
              <p
                className="udesc"
                style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.78rem", fontWeight: 300, color: "rgba(250,250,248,0.7)", lineHeight: 1.75, marginBottom: "1.2rem", opacity: 0, transform: "translateY(12px)", transition: "all 0.4s ease" }}
              >{u.desc}</p>
              <span
                className="ubtn"
                style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.6rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "#C8A96E", border: "1px solid rgba(200,169,110,0.4)", padding: "0.6rem 1.3rem", display: "inline-block", transition: "all 0.3s", background: "transparent" }}
