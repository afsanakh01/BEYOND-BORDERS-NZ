...
export default function Process() {
  return (
    <section id="process" style={{ background: "#080f18", padding: "8rem 4rem" }}>
      <p style={{
        ...
        {steps.map((s, i) => (
          <div key={i} style={{ textAlign: "center" }}>
            <div style={{
              ...
              background: "#080f18", position: "relative", zIndex: 1,
              transition: "all 0.3s",
            }}
              onMouseEnter={e => {
                (e.currentTarget as HTMLDivElement).style.background = "#C8A96E";
                (e.currentTarget as HTMLDivElement).style.color = "#0D1B2A";
              }}
              onMouseLeave={e => {
            ...
            >{s.num}</div>
            <div style={{ fontWeight: 500, fontSize: "0.9rem", color: "#FAFAF8", marginBottom: "0.6rem" }}>{s.title}</div>
            <p style={{ fontSize: "0.8rem", color: "rgba(250,250,248,0.6)", fontWeight: 300, lineHeight: 1.7 }}>{s.desc}</p>
          </div>
        ))}
      </div>
    </section>
